...
- Raw observations land in `docs/personal_problems.md`, a deliberate **inbox** (gitignored — it lives only in the working tree, so never link to it from a tracked doc): write the problem down in whatever words come out, then *triage* it into a P-number, a question, or a task and move it to that file's **Triaged** log with a pointer. It is never spec and holds no rulings. An entry left in **Open** is a review finding, not a backlog item — a doc pass that walks past that file has missed something.
