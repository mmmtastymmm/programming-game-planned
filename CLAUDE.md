...
- Raw observations land in `docs/INBOX.md`, a deliberate **inbox**: write the problem down in whatever words come out, then *triage* it into a P-number, a question, or a task and move it to that file's **Triaged** log with a pointer. It is never spec and holds no rulings. An entry left in **Open** is a review finding, not a backlog item — a doc pass that walks past that file has missed something.
