...
flowchart TD
    ...
    GCHIP -->|top-tier compute:<br/>high Processor tiers, Backup Core| SINK4
...
| **Gold Chip** | 1 Chip + 1 Gold (Foundry) | Top-tier compute — the highest Processor tiers and the Backup Core price in these ([06-progression.md](06-progression.md)) | *Is your colony rich enough to think this hard?* The best brains are gilded. |
...
**Building tier 2 is the ladder's one exception** (Q84, restated for Q105's capability model): the civil-kit tier prices in **Steel**, so the first heavy builder is a free print plus a Steel upgrade, and the Smelter is never locked behind the Bronze it would produce. (Every print already holds Building tier 1, so ordinary `build()` never waits on anything.)
...
- **A completing structure SHOVES whatever stands on its tile; entombment kills** (2026-07-27, answers Q120). A pending designation is **not solid** and never becomes so — making blueprints block pathing was rejected outright because it hands every player a free terrain weapon: designate, never build, and wall off any ground you like. Bots therefore walk over designations freely, and the build that completes on an occupied tile **shoves** the occupant to a free adjacent tile (chosen in sorted order, so replays agree), replanning its in-flight action the way a bump already does. If there is **no** free adjacent tile the occupant is **destroyed and drops a black box**, skipping the wreck stage entirely — there is no wreck to rescue because it is under a building. This is knowingly a **cross-faction kill primitive**: an enemy finishing a build on a cornered bot of yours kills it. That is judged acceptable because it costs a designation, its full typed materials, and the build's whole labour, against a target that must stay cornered for all of it. What the ruling forbids is as important as what it allows: the build must **never hold** (a silent stall mints XP and progress for no output; a stall that faults grinds the builder to a wreck under Q109's fault damage) and must **never delete the designation**, which destroys the player's up-front materials. Both were tried during M16 and both were wrong.
