...
- **A completing structure DISPLACES whatever stands on its tile — nothing dies** (2026-07-27, answers Q120; entombment removed 2026-07-28). A pending designation is **not solid** and never becomes so — making blueprints block pathing was rejected outright because it hands every player a free terrain weapon: designate, never build, and wall off any ground you like. Bots therefore walk over designations freely, and the build that completes on an occupied tile **displaces** the occupant to the nearest free tile.
  ...
  - **Nothing dies.** An earlier draft of this ruling killed an occupant that had no free *adjacent* tile, dropping a black box and skipping the wreck stage — which broke [02-agents.md](02-agents.md)'s "there is no instant-destruction path", left the black box's landing tile undefined under a solid building, and handed every faction a cross-faction kill primitive. Widening the search to the whole map removes all three at once: the case the death rule existed for cannot arise, because a colony's fleet cap sits far below the map's tile count, so a legal game state always has a free tile somewhere. **docs/02's death-path invariant stands untouched.**
  - **The displaced bot is not punished.** Its in-flight path is invalidated, so a `move_to` re-paths from the new tile on its next tick and anything else fails its own range check normally — no fault, no HP chip for being pushed, and no terrain trigger on landing (a displacement is not a walk, so no Ice slide and no Dune reset).
