...
| `closest(kind)` → `Result` | 3 | **yes** | Generic nearest-of-kind query, **query-radius** limited ([05-terrain.md](05-terrain.md)); `Result.Ok(entity)` / `Result.Err(msg)` |
| `exists(kind)` → bool | 1 | **yes** | Any entity of `kind` within the **query radius**? |
