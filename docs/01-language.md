...
| **`bank_cap`** | derived | Max banked cycle budget — computed **per bot per tile** as the most expensive *effective* op cost including the bot's own overlays (25 on base tables; Q75/Q82). Clamped after each grant (see Execution Model) |
...
- **Banked cycles are capped** — budget clamps to `bank_cap` after every grant, and `bank_cap` is **derived per bot per tile** (the most expensive effective op cost including the bot's own overlays — Q75/Q82; all costs bounded by the payload and upload caps); debt from forced charges is unaffected (charged, not banked). Freeze-forever in front of an unaffordable op is impossible by construction, so overlays may raise costs freely.
