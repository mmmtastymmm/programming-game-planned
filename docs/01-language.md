...
| `send(ch, val, timeout=None)` | 3 + size, blocks | no | Requires Tier 7; one receiver, rendezvous; `timeout=None` blocks forever, expiry faults. Channel-blocking calls are never signal-safe (action-blocking ones like `move_to` can be) |
