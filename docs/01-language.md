...
2. **Over-capacity** — the colony exceeds what it can sustain ([02-agents.md](02-agents.md)): the lowest-total-XP bot in the whole colony is recalled **for scrap** (partial Steel refund).
...
- Its bots become **stragglers** — unclaimed by definition. Other printers' selection keys claim the ones they want; the rest fall to the first printer's remainder and are recalled home for re-coloring, XP kept. Whether a deliberate leave-them-be **ghost fleet** (frozen code, no reinforcements, dying by attrition) survives the remainder bucket's auto-absorb is open — Q65.
- Retake the nest and the printer reactivates: cap contribution and target restored, color unfrozen.

Intel wrinkle (live while Q65 is open): a ghost fleet can still be salvaged toward its color's decryption — and since the code can never be rewritten, whatever the enemy has learned about a dormant color is *permanently accurate*.
...
- **`import haul` / `from haul import go_home` (DECIDED — Python's two forms).** `import haul` makes qualified calls (`haul.go_home()`) available; `from haul import go_home` binds the named functions bare (comma lists allowed). Imports are top-level declarations, per program window. Getting it wrong is a **deploy error with a pointed message**: unknown module, unknown function in a `from` list, a qualified call on a module never `import`ed, and alias/def name collisions all fail the deploy. Unlock placement stays open (Q61). A call to an imported function costs exactly what any `def` call costs (2 + body). The `import` line itself is free at runtime because it doesn't exist at runtime:
...
| `salvage(entity)` | 2 + action | no | Recover partial Steel from a wreck (works on anyone's); destroys it → drops its Black Box. Salvager also gains **+N% permanent decryption of the bot's program color** (default 5%, [08-multiplayer.md](08-multiplayer.md)) |
...
| `search()` | 2 + action | no | Prospect: the bot roots in place and surveys outward one ring per N ticks (tuning) out to its **query radius** (sensor range × `query_factor`, [05-terrain.md](05-terrain.md)), discovering buried resource nodes ([03-resources.md](03-resources.md)); each new node earns Scouting XP. Moving or any signal ends the survey |
...
- **Abort is fully engine-reserved** (like recall): no `on abort:` window exists. Every death runs the forced `upload_log()` then `become_disabled()` — the logs always reach the cloud, and the wreck's self-destruct countdown starts: field-repair in time rescues it (XP intact); expiry is the **only** way a bot explodes. There are no last words — the black box is whatever the bot logged while alive. Wrecks can also be `salvage()`d (by anyone) for partial Steel.
...
- **Queries reach past the eyes; undiscovered veins don't answer** (2026-07-14, answers Q57; refined same day — [05-terrain.md](05-terrain.md)): queries range over the bot's **query radius**, derived larger from the same sensor-range stat that reveals fog (× `query_factor`, tuning ~150%) — sensing outranges seeing, so any beyond-eyes query may return an entity on a still-fogged tile; per-kind bonuses (Combat L3, Ore-acle) extend the *query* for that kind only, on top. For resource kinds, `closest`/`exists` see tier-0 surface nodes on sight but buried (tier-1+) nodes **only after the colony has prospected them** with `search()` — an unsurveyed vein is a miss, not a secret hit.
