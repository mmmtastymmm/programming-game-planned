...
- **Bank cap — derived, not configured (Q75).** The budget clamps to **`bank_cap`** after every grant, and `bank_cap` is *computed per tile*: the most expensive **effective** op cost under the local overlay stack (base tables give 25, the crash dump; inside Corruption it's 26; on an Overclock Field, 50). Freeze-forever in front of an unaffordable op is now impossible *by construction* — the cap always reaches the priciest op wherever the bot stands — so no load-time overlay rule is needed, and the inspector's budget meter simply rescales per biome. Saving up still can't overshoot the op you're waiting for.
...
- **Entry defers until `Running`.** The allocation records the bot's new assignment as state; the recall interrupt *enters* only when the bot is in plain Running — never mid-handler, mid-boot, or mid-recall. Your own bookkeeping cannot double-handle your bots; the walk home stays the risk, entry never is.
- **Re-targets are absorbed.** A bot already walking home that gets reassigned again just re-targets its walk (the handler is engine-owned code — changing its destination is not a second signal). If the new assignment matches the bot's *current* color, the recall cancels and the program restarts at line 1 in place — no boot, since no re-coloring happened.
...
| `closest(kind)` → `Result` | 3 | **yes** | Generic nearest-of-kind query over what the bot perceives — everything within **seeing**, movers within **hearing**, discovered nodes from map knowledge ([05-terrain.md](05-terrain.md)); `Result.Ok(entity)` / `Result.Err(msg)` |
...
| `analyze(wreck)` | 2 + action | no | The **intel verb** (Q76): dissect any faction's wreck — yields **Data**, reads its logs + env snapshot, and extracts its faction's **comm key**; Feral wrecks also grant per-arcanum decryption ([04-enemies.md](04-enemies.md)). Destroys the wreck. Materials or intel, pick your verb — `salvage()` is the economy one |
...
| `search()` | 2 + action | no | The **scouting stance** ([05-terrain.md](05-terrain.md)): the bot roots in place and its *seeing* circle expands one ring per N ticks (tuning) out to its hearing radius — full sight at range, geology included; each new node discovered earns Scouting XP. Moving or any signal ends the stance |
...
- **Generic fallible queries** — `exists(kind)` / `closest(kind)` with bare kind constants (`ore`, `depot`, `enemy`, `blueprint`, …) replace the old `nearest_*` / `*_exists` builtin family. `closest` returns the builtin `Result` enum (`Result.Ok(entity)` / `Result.Err(msg)`); unwrap with the `.expect()` method (faults with the message on `Err` — same behavior the old builtins had on a miss) or handle fault-free with `match`. Kind constants are host-bound names, shadowable, and survive post-fault restarts. Per-raw constants (`iron` … `sand`) query one kind; `ore` is the family constant for any discovered mineral vein (Q69). **Yields are typed**: `mine()` produces the node's specific resource — cargo is a typed manifest, never generic "ore units" — so hauling programs route by kind and Smelter/Foundry buffers accept only their recipe's inputs.
- **Queries follow the seeing/hearing circles** (2026-07-14, Q74 — supersedes Q57's query-radius phrasing; [05-terrain.md](05-terrain.md)): full returns within **seeing** (the sensor-range stat — total information); **movers only** within **hearing** (× `sense_factor`, ~150% — only moving things make noise, so a heard contact is a position, not a dossier); plus **discovered resource nodes from map knowledge at any range** (a known vein is a fact, not a perception — an undiscovered vein is still a miss, not a secret hit). LoS blocks both circles. Per-kind bonuses (Combat L3) widen the hearing circle for their kind.
