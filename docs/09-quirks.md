...
- Roll at print time, `quirk_roll` RNG stream, weighted by `quirks.ron` rarity — but rolls are **latent**: a quirk manifests (effect + visibility + introspection) only when the bot's **Age LEVEL** reaches its threshold (Q105 ruling (a), restated 2026-07-28 — see Decided). Manifestation is a deterministic threshold check, no RNG.
