...
- Roll at print time, `quirk_roll` RNG stream, weighted by `quirks.ron` rarity — but rolls are **latent**: a quirk manifests (effect + visibility + introspection) only when the bot's **Age-track** XP crosses its threshold (Q105 ruling (a) — see Decided). Manifestation is a deterministic threshold check, no RNG.
...
- **Quirks manifest with experience, not at print** (same day, refinement). The roll happens at print but stays **latent** — no effect, not visible, not introspectable — until the bot's **Age-track XP crosses a threshold** (Q105 ruling (a), 2026-07-26: keyed to Age rather than total XP, because tier-scaled task XP would cross the old 300/900 totals within a few units of work and pop every latent quirk at once — Age is tier-independent, cannot be farmed, and is still zero on a fresh print. Thresholds retune against Age's 1-per-10-ticks accrual; tuning in `quirks.ron`). A fresh print is always quirk-free, so **kill-and-reprint reroll-fishing buys nothing**: with prints defaulting to free, a print-time roll would be a slot machine — this way every look at a new roll costs raising a bot to the threshold, and rerolling means losing that XP (pillar 3 does the anti-fishing work). Thematic bonus: machines develop personality as they wear in — quirks join the scar-tissue family, rolled at birth but revealed by a life.
