...
| Compute | **Cycles per tick** | 1 | **CPU tool** (Upgrade Station), licensed by the **Processing track** (operations executed) | Q100 retired the Coprocessor: thinking and acting never overlap, because "think while acting" is a *language* feature (async actions), not hardware. Compute instead joins Q105's capability model — buy the tier, then sharpen it by working. |
...
- **Compute is bought at the Upgrade Station; passive stats level by happening** (2026-07-13). All six compute stats upgrade per-bot at a player-placed **Upgrade Station** the bot must physically walk to (Chips — [03-resources.md](03-resources.md), catalog in [06-progression.md](06-progression.md)); compute is now the **CPU tool**, licensed by the Processing track like every other tool (Q111). Use-based **body tracks** (Hiding, Flinch, Age, Mileage, Processing) level the passive stats by the event itself happening — guarded **where farming would be free** (2026-07-14, restated by Q116): hostile-source flinches only, detection per-episode with escape re-arm. Boot and Learning were later cut. Two tracks were cut the same day: **Print** (no bot is printed twice — could never level) and **Regen** (self-hurt-then-heal was a free XP machine; self-repair growth folded into Age).
