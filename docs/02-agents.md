...
| Survival | **Countdown** | ~20 s + **1 s per 100 total XP** (tuning, Q83) | total XP (built into the base formula) | The wreck's self-destruct timer: your rescue window, their salvage/analyze/hijack window. A rookie pops in ~20 s (rescue radius ≈ 14 tiles at stock speed — perimeter rescues only); a maxed veteran lingers ~2.75 extra minutes — the richest prizes give everyone the most time to fight over them. |
...
- **Population is capped by printers, priced by the economy.** Every printer adds a fixed amount to the colony's **fleet cap** (tuning, `printers.ron`) — territory (nests → printers, [04-enemies.md](04-enemies.md)) is the only hard ceiling on fleet size. Below that ceiling, the constraint is economic: upkeep is a **data-driven resource mix** (an `upkeep.ron`-style config, adjustable without code changes — prototype the system, then tune); **v1 config: Energy (primary drain) + Steel (chassis maintenance)** per [03-resources.md](03-resources.md). Over-extending doesn't block printing — it degrades the colony (brownout halves cycle budgets) and, if sustained, triggers **scrap recalls**: the colony recalls its lowest-total-XP fleet bot for a partial build-receipt refund. The fleet's real operating point is an economic equilibrium the player feels; the printer-derived cap is only the ceiling above it.
...
- **Q100** ([QUESTIONS.md](QUESTIONS.md)): the inert hardware — Coprocessor (think-while-acting needs a VM concurrency design) and Backup Core (XP preservation needs a death-rework ruling); design both or pull them from the v1 catalog.
- **Q105** ([QUESTIONS.md](QUESTIONS.md)): the build tool — Q84 priced it in Steel but nothing implements it, so repair/hijack ship ungated and claim/raze stay instant Commands; decide its landing and which acts require it.
