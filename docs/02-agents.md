...
| Senses | **Sensor range** | 5 tiles | Scouting: +1/level; Combat L3: +1 vs enemies | **One stat, two radii** (Q57): eyes-only fog reveal = this value; the query radius (`closest()`, `exists()`, `scan_*()`, `search()`) is derived larger — × `query_factor` (tuning ~150%: base 5 eyes / 7 queries) — so sensing outranges seeing and every point here widens both. Per-kind bonuses extend queries only; beyond-eyes queries return entities on fogged tiles ([05-terrain.md](05-terrain.md)). A blind bot isn't dumb — it's uninformed, and no cycle budget fixes that. (Where base sight lives: Q53.) |
