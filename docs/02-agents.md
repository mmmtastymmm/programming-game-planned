...
| **Compute upgrades** | Program memory, variable slots, stack depth, log buffer, and the Backup Core — flat per-bot buys at an **Upgrade Station**; **cycles per tick is a capability** (tier × level, Q100/Q105) rather than a flat buy ([03-resources.md](03-resources.md)), a player-placed structure the bot must physically walk to. Compute never occupies a module slot. |
| **Capability slots** | Four permanent slots — **Mining, Building, Combat, Optics** — one per thing a bot does (Q105, 2026-07-26; supersedes generic module slots). Each carries a **tier** (bought at the Upgrade Station: what it can reach — mining tier N works resources of tier ≤ N, [03-resources.md](03-resources.md)) and a **level** (earned by *using* it: how well — the task-track perks). Nothing is ever locked out: base tier is 1, so a fresh print works its whole start zone. **A tier change effectively resets that capability's level** — new tool, new hands — but by arithmetic, not erasure: each tier scales its level thresholds *and* its XP gain, so old XP goes negligible against the new ladder while re-climbing costs the same work. Upgrading is a deliberate dip, never a longer grind, and nothing is taken away on paper. Language unlocks are colony-wide; capabilities are per-bot. |
...
Five sources feed the sheet, in modifier-pipeline order: **universal base** (the identical print floor) → **hardware** (Upgrade Station purchases + slotted modules, each priced per its catalog) → **XP perks** (earned) → **quirks** (rolled/acquired) → **state** (temporary: Damaged, brownout, Corruption, loaded, terrain).
...
| Compute | **Cycles per tick** — see the Processor capability | 1 | **tier** (Upgrade Station) × **level** (the Processing track: executing operations) | Q100 retired the Coprocessor: thinking and acting never overlap, because "think while acting" is a *language* feature (async actions), not hardware. Compute instead joins Q105's capability model — buy the tier, then sharpen it by working. |
| Survival | **Backup Core** | absent | Upgrade Station (late, Gold Chips) | Preserves every capability **tier** into the reprint and **wipes all XP** (Q100): it protects what you *bought*, never what you *earned*. No re-outfitting, no re-buying hardware — but the machine earns its proficiency back from zero, so death keeps its sting. Preserves itself, so it re-arms once bought. |
...
| Body | **Capability tiers** | 1 each | **bought** at the Upgrade Station (Q105) — five slots with the Processor (Q100) | Four permanent slots — Mining, Building, Combat, Optics — each a tier that sets what the capability can *reach* (mining tier N works resource tier ≤ N). Base 1 means a fresh print works its start zone (Wood/Stone/Sand/Iron/Coal); tier 2 unlocks Copper/Tin, which is also where mapgen's midfield band begins, so hardware and geography climb together. *(Supersedes Q66's generic module slots and their 1,000/3,000 XP milestones — levels now come from use, which was Q66's spirit.)* |
| Body | **Capability levels** | 0 each | **use**: the paired task track (Mining/Building/Combat/Scouting) | How WELL each capability performs — the existing track perks (+10% yield, +10% build rate, +5% damage, +1 sensor per level) read as proficiency with the tool in hand. **A new tier effectively resets the level, by scale rather than erasure**: each tier multiplies that capability's level thresholds *and* XP gain by `M` (tuning). The curve is cumulative 100/300/600/1000/1500, so any `M > 15` puts even a maxed tier-1 bot below the new L1 — and since gain scales too, re-climbing costs the same *work*. `M` is the mastery-carry-over dial (at 20 a maxed veteran starts 75% toward the new L1; at 100, 15%). XP never decreases, so a veteran always reads as a veteran to selection keys and the scrap valve. **Q105-R1: a tier's own grant must dominate the levels it resets** (tier N's flat bonus ≥ the L5 bonus of tier N−1, checked at load) — otherwise buying Optics tier 2 would reset Scouting and leave a maxed scout seeing *less* than before, a strict loss rather than the intended dip. |
| Senses | **Sensor range** | 5 tiles | **Optics tier** (bought) + Scouting level: +1/level (both circles); Combat L3: +1 vs enemies (hearing only). Per Q105-R1 a tier's grant must dominate the levels it resets, so buying Optics can never leave a scout seeing *less* | **One stat, two circles** (Q74): **seeing** (this value — total information: fog lifted, entities tracked, geology known) and **hearing** (× `sense_factor`, tuning ~150%: base 5 sees / 7 hears — detects *movers only*; the ground stays fogged). Only moving things make noise. Queries return everything within seeing, movers within hearing, and discovered nodes from map knowledge ([05-terrain.md](05-terrain.md)). A blind bot isn't dumb — it's uninformed, and no cycle budget fixes that. (Q53 answered: base sight is innate — no bot is ever blind; **Optics is a permanent capability slot** since Q105, starting at tier 1 and bought upward with Lens-built hardware — it no longer competes for a generic module slot, so no rookie ever trades its working ability for eyes.) |
| Senses | **Signature** | 0 | **Hiding track**: per **detection episode** — XP on being seen *or heard* by an enemy faction, re-armed after fully escaping both for M ticks (−1 signature/level, tuning) | How loud this bot is (Q54, reworked under Q74): a *moving* bot is heard at `their hearing radius + this signature`, floored at 1. Noisy (+, Loud Fans) is heard from beyond the normal circle; quiet (−) must be approached. **Sight is absolute** — inside the seeing circle nothing hides — and a stationary bot makes no noise at all. **Creeping** is the active half (Q103): `creep=True` on any pathing call buys a signature cut with much slower travel, so a careful bot is heard only from much closer. The formal home for every "enemies sense this bot at ±N" effect. |
| Tools | **Damage** | **Combat tier 1** (every print has one — weapon modules died with Q105's generic slots) | Combat tier (bought) + Combat level: +5%/level | Kill speed — which in the wreck economy is also *rescue-denial* speed. |
...
| Survival | **Boot ritual** | ~20 ticks of engine time | **Boot track**: rescue boots from **hostile-caused wrecks** only — self-scuttles, friendly accidents, prints, and re-colors grant nothing | The fixed engine portion of every print, rescue, and re-coloring; the forced `upload_log()` charges as engine debt, then any `on boot:` window pays normally (Q75). All of it is double-handle exposure — boot time is rescue risk. |
| Survival | **Countdown** | ~20 s + **a per-Age bonus** (tuning, Q83; Age not total XP since Q105 — tier-scaled totals would push a mid-tier bot's countdown into the hours and its wreck would never expire) | the **Age** track | The wreck's self-destruct timer: your rescue window, their salvage/analyze/hijack window. A rookie pops in ~20 s (rescue radius ≈ 14 tiles at stock speed — perimeter rescues only); a maxed veteran lingers ~2.75 extra minutes — the richest prizes give everyone the most time to fight over them. |
...
| Survival | **XP preserved** | 0% on destruction | — (permanent: Q100 ruled the Backup Core preserves TIERS, not XP) | What a reprint inherits. Experience is never inherited — the Backup Core softens pillar 3 by carrying the *hardware* across, so a rebuilt veteran is fully equipped but green ([06-progression.md](06-progression.md)). |
| Survival | **Salvage profile** | +N% color decryption to the salvager (the match constant, default 5% — [08-multiplayer.md](08-multiplayer.md)); a cut of the **build receipt**; reprint at full cost | *derived* (Q68): the build receipt — a fraction of every material **currently installed** (the chassis line if the print was priced, slotted modules, bought compute; swapped-out modules drop off — Q72 swap rules) | What your corpse is worth to the enemy — and what the replacement costs you. Scales with the bot: the more it has become, the juicier the kill — and it *looks* juicy (enemy-visible, pillar 2). |
...
- **Print cost is a match setting, default FREE** — a colony must never be soft-locked (no resources + no bots = no way to gather resources). Population stays bounded by printer dials and colony capacity. **The build receipt is literal (Q72)**: refunds and salvage return fractions of what was *actually spent* on the bot — a free print's chassis line is 0, while upgrades and modules (never free) always count. "Scrapping never mints resources" holds automatically under every print-cost setting; rookie wrecks are worth ~nothing plus decryption, veteran wrecks are rich.
- Fabricators keep a **reprint queue** UI: destroyed bots appear with one-click requeue — a convenience counter for "my fleet lost a body," producing a fresh stock print whose color comes from the allocation (Q73). Nothing per-bot is resurrected.
- Late unlock: the *Backup Core* — expensive (Gold Chips), and per Q100 it preserves every capability **tier** into the reprint while **wiping all XP**. What you bought survives; what you earned does not. Gated late so early losses stay meaningful ([06-progression.md](06-progression.md)).

## Decided

- **One universal chassis — no classes** (2026-07-13, supersedes the Scamp/Drudge/Bulwark/Artisan table). Every print is identical, starting at the floor of the old class options (HP 40, move 14 ticks/tile, cargo 4, sensors 5, slots 1 — all tuning). Specialization is earned, never printed: XP tracks, modules, and quirks are the only differentiation. Anything that was class-gated re-gates on **capability tiers** (field repair, hijack and nest conversion need Building tier ≥ 2 — Q105-R2; tool modules themselves died with Q105's generic slots).
