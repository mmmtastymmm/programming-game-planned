...
| Body | **Module slots** | 1 | **total-XP milestones**: +1 slot at thresholds (tuning), cap 3 | Every bot starts with one slot, so *which* module fills it is the identity decision — chosen at print (the Fabricator slots it, materials added to the print) or swapped later at the Upgrade Station (Q72). Veterancy literally builds out the frame: total XP across all tracks unlocks the 2nd and 3rd slots — the old chassis range, now earned. |
