...
| `try_move_to(target, only=None, avoid=None, creep=False)` → bool | 2 + travel | The fault-free walk: takes a concrete entity or position, and an **unreachable goal** is no action and `False` rather than a fault. It does *not* accept a `Result` or an `Option` — unwrap first (guard with `exists`). Start kit |
...
| `wander(only=None, avoid=None, creep=False)` | 2 + action | A seeded random walk leg (stream `rng.wander`) — the dumb explorer; takes the Q95 paint args (`wander(only=green)` = drift inside the green zone). Start kit |
| `explore(only=None, avoid=None, creep=False)` | 2 + action | The smart explorer (Q79): picks a random **currently-fogged** tile within ~15 tiles (tuning; stream `rng.explore`), walks there under the Q95 paint args, and drops into the scouting stance; resolves when the survey completes |
...
| `analyze(wreck)` | 2 + action | The **intel verb** (Q76): dissect a wreck — from **other factions' wrecks** it yields **Data**, reads logs + env snapshot, and extracts the faction's **comm key**. It grants **no decryption** — that rides `salvage()` alone, for Feral and player wrecks alike ([08-multiplayer.md](../08-multiplayer.md)). **Your own wrecks yield nothing** (no staged Data — you already own their logs and key). Destroys the wreck. Materials or intel, pick your verb — `salvage()` is the economy one |
