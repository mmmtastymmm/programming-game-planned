...
- **The editor still tells you what it costs.** Each `def` carries its **worst-case instruction count** where that is computable (longest branch, calls expanded) and reads **unbounded** where a loop or recursion makes it undecidable. This is information, not a gate — nothing is greyed out. One case earns a deploy-time *warning* because it has no natural teaching loop: an **unbounded window** — a loop the analysis can't bound, or a **channel** call left to block forever (`timeout=None`) — parks the bot inside the template indefinitely (an *action*-blocking verb like `move_to` always resolves, so it never warns), producing neither a wreck nor a crash dump to learn from.
