...
- **Blocking consumes cycles.** A blocked bot (send *or* receive) executes nothing else, and its per-tick grant is forfeited while it waits — waiting *is* what its CPU is doing. No banking cycles, no free listening posts: a bot blocked for 100 ticks spent 100 ticks of compute on patience. (What burns is the *grant*; cycles banked before the block are retained — see [execution-model.md](execution-model.md), which owns the rule.) Handlers still fire while blocked — with the usual double-handle stakes.
