...
- Handler templates render as the full sandwich: forced prologue/epilogue lines shown locked (phantom lines, not in your source) around the editable window. Nothing inside is greyed out — the window takes any code — but each `def` shows its worst-case instruction count (or **unbounded**), and an unbounded window draws a warning.
