...
- **Kind constants** — pre-bound global constants naming entity kinds; the generic queries take them as arguments (`closest(ore)`, `exists(blueprint)`). Assignments may shadow them (ordinary names, unlike the reserved `None`/`True`/`False`) and they survive post-fault restarts. **Every resource and every registry kind gets one** (Q79, completed round 4):
  ...
  - **Battlefield objects**: `wreck`, `black_box`, `blueprint`, `cache`, `nest`, `blight`, `barricade`; **bots**: `enemy` / `ally`. What a query over one of these *reaches* is not settled here — the domains are owned by [decided.md](decided.md) (Q74's circles, P22's structure pool) and [builtins.md](builtins.md), and differ by kind. Two entries carry a note the registry itself must hold:
    - **`blight`** (the creep's heart) is the one **perception-ungated** placement — the creep front is visible terrain, so `closest(blight)` answers without eyes on it.
    - **`barricade`** exists as a constant by Q79's rule, on Q99's grounds ("attackable, so it must be findable"), but is **not yet in the shipped registry**: the entry is blocked on **Q127**, which rules its query *domain* (P29 — whether a rival's wall is reachable at all), never its existence.
