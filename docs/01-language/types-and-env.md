...
- **Kind constants** — pre-bound global constants naming entity kinds; the generic queries take them as arguments (`closest(ore)`, `exists(blueprint)`).
- **Faction-selector constants** (P22, 2026-08-02) — pre-bound like kind constants (the Q79/Q80 mechanism): **`own`**, **`ally`**, **`enemy`**, **`any`**. They are the value domain of the queries' `faction=` parameter ([builtins.md](builtins.md)) — legal only with structure and designation kinds. `send`'s `faction=` parameter is a **different domain**: it takes `own` or a specific faction handle, never a selector — a selector where a handle is expected is a type fault. Assignments may shadow them (ordinary names, unlike the reserved `None`/`True`/`False`) and they survive post-fault restarts. **Every resource and every registry kind gets one** (Q79, completed round 4):
