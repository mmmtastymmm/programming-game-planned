...
- **Cycle costs are layered data**: base `costs.ron` asset (op → cost, including `fault_penalty`) + per-map/biome **overlay** assets ([05-terrain.md](05-terrain.md)). Effective cost = overlay(base) for the tile the bot occupies, resolved at step time. Overlays are moddable content — validate at load (every key must exist in base; costs ≥ 0; **no effective cost above `bank_cap`**, else a bot could freeze forever in front of an op it can never afford) so a bad mod fails loudly, not as a desync or a soft-lock.
