...
- **Cycle costs are layered data**: base `costs.ron` asset (op → cost, including `trap_cost`) + per-map/biome **overlay** assets ([05-terrain.md](05-terrain.md)). Effective cost = overlay(base) for the tile the bot occupies, resolved at step time, **floored at 1** (Q75 — no zero-cost ops, no infinite intra-tick loops). Overlays are moddable content — validate at load (every key must exist in base; costs ≥ 1) so a bad mod fails loudly. The old above-`bank_cap` check is gone: `bank_cap` is **derived per bot per tile** as the max effective op cost including the bot's own overlays (Q75/Q82 — with payload/upload caps bounding sized costs), so freeze-forever is impossible by construction ([01-language.md](01-language.md)).
