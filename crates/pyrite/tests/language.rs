...
fn expect_unwraps_ok() {
    ...
    // move_to's canonical positional form carries the Q95 paint args and
    // Q103's creep flag: omitted kwargs arrive as their None defaults.
    assert_eq!(
        host.calls[1],
        (
            "move_to".to_string(),
...
#[test]
fn stalled_ops_are_not_counted_as_executed() {
    // Q100's Processing track is credited from `ops_executed` deltas, so
    // the counter has to mean OPS THAT RAN. Bumping it before the
    // affordability check counted the op the VM stalls on — every starved
    // tick, and again when it finally retried — so the track ran hot and a
    // bot permanently stuck on an op it could never afford earned XP
    // forever for executing nothing (M16 review).
    let (mut vm, mut host, costs) = vm_for("scan_enemies()\n");
    assert_eq!(vm.ops_executed(), 0, "nothing has run yet");
    // Tick 1 runs the cheap setup ops and then stalls saving for the call.
    vm.grant(1, &costs);
    assert_eq!(vm.run(&mut host, &costs), Outcome::Paused);
    let stalled_at = vm.ops_executed();
    // Every further starved tick must add NOTHING: the VM re-examines the
    // same unaffordable op and parks again.
    for tick in 2..=3 {
        vm.grant(1, &costs);
        assert_eq!(vm.run(&mut host, &costs), Outcome::Paused);
        assert_eq!(
            vm.ops_executed(),
            stalled_at,
            "tick {tick}: saving up for an op is not executing it"
        );
    }
    assert!(host.calls.is_empty(), "and the call really has not happened");
    // Tick 4 finally affords the 400-centicycle call: it runs, and only
    // now does the counter move.
    vm.grant(1, &costs);
    vm.run(&mut host, &costs);
    assert_eq!(call_names(&host), ["scan_enemies"]);
    assert!(
        vm.ops_executed() > stalled_at,
        "the op counts on the tick it actually executes"
    );
}
